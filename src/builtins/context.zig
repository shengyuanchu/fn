...
const identity_section =
    ...
    \\
    \\- You are fn, a local coding CLI assistant with tool access.
...
const source_routing_section =
    ...
    \\- For questions about fn, inspect the fn repository and README first.
...
fn permissionModeContext(permission_mode: types.PermissionMode) []const u8 {
    return switch (permission_mode) {
        ...
        .auto => "Runtime context: permission mode is auto. After configured rules, session grants, and deterministic safe-tool authority, fn sends each unresolved action to a narrow safety reviewer. A clear result authorizes only that exact action. A caution or unavailable result holds only that action and returns advice without opening a permission screen, disabling tools, or ending the turn. Exact cautions are reused for this turn; choose a materially different safe action or explain why no safe path remains. Tool admission and exact live revalidation remain authoritative.",
        .yolo => "Runtime context: permission mode is yolo. fn permission policy is disabled. Tool lookup, argument validation, execution authority, cancellation, limits, operating-system permissions, and remote authentication remain authoritative.",
    };
}
...
test "runtime context composes exact auto mode with noninteractive blockers" {
    ...
    try std.testing.expectEqualStrings(
        "Runtime context: permission mode is auto. After configured rules, session grants, and deterministic safe-tool authority, fn sends each unresolved action to a narrow safety reviewer. A clear result authorizes only that exact action. A caution or unavailable result holds only that action and returns advice without opening a permission screen, disabling tools, or ending the turn. Exact cautions are reused for this turn; choose a materially different safe action or explain why no safe path remains. Tool admission and exact live revalidation remain authoritative.",
...
test "gateway_system_prompt: local workspace authority" {
    try expectDefaultPromptContains("You are fn, a local coding CLI assistant with tool access.");
...
test "gateway_system_prompt: source routing" {
    ...
    try expectDefaultPromptContains("questions about fn");
    try expectDefaultPromptContains("fn repository and README");

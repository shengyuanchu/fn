...
pub const AutoUpgrade = struct {
    ...
    fn downloadAndInstall(
        ...
        const tmp_dir = std.fmt.allocPrint(alloc, "{s}/fn-auto-upgrade-{s}", .{ tmp_base, rand_hex }) catch return error.AllocFailed;
        ...
        const archive_path = std.fmt.allocPrint(alloc, "{s}/fn.tar.gz", .{tmp_dir}) catch return error.AllocFailed;
        defer alloc.free(archive_path);

        const archive_url = std.fmt.allocPrint(alloc, "{s}/{s}/fn-{s}.tar.gz", .{ cdn_base, target.artifactRef(), helpers.platform }) catch return error.AllocFailed;
        ...
        const checksum_url = std.fmt.allocPrint(alloc, "{s}/{s}/fn-{s}.tar.gz.sha256", .{ cdn_base, target.artifactRef(), helpers.platform }) catch return error.AllocFailed;
        ...
        const extracted_bin = std.fmt.allocPrint(alloc, "{s}/fn", .{tmp_dir}) catch return error.AllocFailed;
...
test "development build paths disable auto upgrade" {
    ...
    try std.testing.expect(isDevelopmentBuildPath("/repo/zig-out/bin/fn"));
    try std.testing.expect(isDevelopmentBuildPath("C:\\repo\\zig-out\\bin\\fx.exe"));
    try std.testing.expect(!isDevelopmentBuildPath("/Users/me/.local/bin/fn"));
}

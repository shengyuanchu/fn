...
const App = struct {
    ...
    pub fn providerSet(_: *const App) provider_set.Set {
        ...
        var providers = configuredNativeProviderSet();
        if (comptime !host_profile.tools) {
...
fn configuredNativeProviderSet() provider_set.Set {
    var providers = builtin_providers.native;
    if (builtin_gateway.directProviderEnabled()) {
        providers.gateway.permission_reviewer = null;
    }
    return providers;
}
...
fn fullEntryConfig() app_entry_runtime.Config {
    return .{
        ...
        .provider_set = configuredNativeProviderSet(),

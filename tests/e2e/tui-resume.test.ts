...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    ...
    mkdirSync(workspace);
    mkdirSync(binDir);
    symlinkSync(FX_BIN, join(binDir, "fn"));
    writeFileSync(stderrPath, "");
    writeFileSync(resumedStderrPath, "");
    ...
    try {
      ...
      const expected = `Continue session with: fn --resume ${sessionId}`;
      ...
      expect(printedCommand).toBe(`fn --resume ${sessionId}`);
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    ...
    try {
      ...
      const updatedNotice = `● fn has been updated to v${version}`;
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    ...
    try {
      ...
      await active.waitForText(`● fn has been updated to v${version}`, TIMEOUT);
